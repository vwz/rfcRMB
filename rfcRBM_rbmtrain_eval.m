function [Theta, batchposhidprobs, batchposhidprobs_aux] = rfcRBM_rbmtrain_eval(batchdata, batchdata_aux, batchlabel, batchtau, batchtau_aux, params, Theta, F)
...
for epoch = epoch:maxepoch,
    ...
    for batch = 1:numbatches,
        fprintf(1,'labeled part: epoch %d batch %d\r',epoch,batch);
	...
	for l=1:N
	    ...
	    err2 = err2 + lambda1 * R1 + lambda2 * L2;
	end
	

	%%%%%%%%% SET MOMENTUM %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        err= sum(sum( (data-negdata).^2 ));
        errsum = err + errsum + err2;
        if epoch>50,
    ...
    for batch = 1:numbatches_aux,
        fprintf(1,'unlabeled part: epoch %d batch %d\r',epoch,batch);
